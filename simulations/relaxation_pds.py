...
def relaxation_pds_prob(G, beta, gamma, initial_infected_nodes, tol=0.001, omega=1.3):
    N = G.number_of_nodes()
    status = np.zeros((N, 3))

    for node in G.nodes():
        if node in initial_infected_nodes:
            status[node, 1] = 1.0  # Infected
        else:
            status[node, 0] = 1.0  # Susceptible
    ...
    while max(i) > tol:
        ...
        for node in G.nodes():
            ...
            prod_term = np.prod([1.0 - beta * i[x] for x in in_neighbors])
            s_new = s[node] * prod_term

            i_new = (s[node] - s_new) + i[node] * (1.0 - gamma)
            r_new = r[node] + i[node] * gamma

            s[node] = s_new
            i[node] = omega * i_new + (1.0 - omega) * i[node]
            r[node] = r_new
...
def test_conditional_probability_iteration_estimation():
    ...
    results = relaxation_pds_prob(G, beta, gamma, initial_infected_nodes, tol, 1.3)
